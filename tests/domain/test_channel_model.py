...
class TestChannelModel:
    ...
    def python_filter(self):
        """Python过滤器配置"""
        return PythonScriptFilterConfig(
    ...
    def test_channel_creation_full_config(self, tcp_source, http_destination, tcp_destination, 
                                        python_filter, python_transformer):
        """测试完整配置创建通道"""
        # Arrange & Act
        channel = ChannelModel(
            id="full-channel",
...
class TestDestinationConfigs:
    ...
    def test_tcp_destination_creation(self):
        """测试TCP目标创建"""
        # Act
        destination = TCPDestinationConfig(
